...
| `IBAN` | grouped or compact, **mod-97-valid** — including when an uppercase word follows (`GB82 WEST … 32 ABCD`), by a retry at the registered IBAN length for the country | other bank identifiers (SWIFT/BIC, UK sort codes); an IBAN glued to trailing text whose country code is not in the ISO 13616 registry |
